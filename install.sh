...
script_header() {
    ...
    echo -e "              (Version 2.6.0) - by: @voltsshx"
...
verification() {
    ...
    if verify_key "$user_key" "$valid_keys"; then
        ...
        echo "-------------------------------------------"
        Certainly! Let's continue the script from where we left off:
        while true; do
            echo -e "\nPlease enter Authentication(AUTH): 👇"
            echo -e "(must be at least 10 characters)"
            read -p "" PASSWORD
...
EOF
        ...
        download_hysteria() {
            ...
            local _download_url="$REPO_URL/releases/download/v${_version}/hysteria-${OPERATING_SYSTEM}-${ARCHITECTURE}"
        ...
        volt() {
            ...
            echo -e "\033[1;32m      ♻️ \033[1;37m      \033[1;33mPlease wait...\033[0m"
            echo -e ""
            wget -O /usr/bin/volt --no-cache 'https://raw.githubusercontent.com/farukbrowser/UDP-Hysteria/main/lib/volt.so' &>/dev/null
            wget -O /etc/volt/cfgupt.py --no-cache 'https://raw.githubusercontent.com/farukbrowser/UDP-Hysteria/main/lib/cfgupt.py' &>/dev/null
